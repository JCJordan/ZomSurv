		...
		for(Event event : events){
			if(event.getLocation().equalsIgnoreCase(location) || event.getLocation().equalsIgnoreCase("Null")){
				...
				if(event.getEnv() == "Z"){
					...
					num = (float)(event.getProbability()*(map.get_inf_dens_point((int)(player.getPosition().getXPos()/4),(int)(player.getPosition().getYPos()/4))/map.get_inf_av())*0.5);//4 there to convert between different sized images
