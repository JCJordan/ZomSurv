...
public class Player extends Mob {
	...
	private int colour = Colours.get(-1, 111, 245, 543);
	private int scale = 1;
	protected boolean isSwimming = false;
	private int tickCount = 0;
	private String username;
	
	boolean questionShow = false;
	...
	public void render(Screen screen) {
		...
		if(input.space.isPressed()) {questionShow ^= true; correctAnswer = null;}		
		
		String question = "The fuck man";
		String answerA = "A";
		String answerB = "B";
		String answerC = "C";
		
		if(questionShow) {
			Font.render(question, screen, (xOffset - (24*8)), (yOffset - (17*8)) + (x*10), Colours.get(-1, -1, -1, 555), 1);
			Font.render(answerA, screen, (xOffset - (24*8)), (yOffset + (10*8)) + (x*10), Colours.get(-1, -1, -1, 555), 1);
			Font.render(answerB, screen, (xOffset - (24*8)), (yOffset + (13*8)) + (x*10), Colours.get(-1, -1, -1, 555), 1);
			Font.render(answerC, screen, (xOffset - (24*8)), (yOffset + (16*8)) + (x*10), Colours.get(-1, -1, -1, 555), 1);
		}
		...
		if(questionShow) {
			...
			if(correctAnswer == "Correct") {
				Font.render("Correct Answer!", screen, xOffset - (("Correct Answer!".length() -1) / 2*8), (yOffset) - (4*8), Colours.get(-1, -1, -1, 555), 1);
			} 
			if(correctAnswer == "Wrong") {
				Font.render("Wrong Answer", screen, xOffset - (("Wrong Answer".length() -1) / 2*8), (yOffset) - (4*8), Colours.get(-1, -1, -1, 555), 1);
			} 
		}
