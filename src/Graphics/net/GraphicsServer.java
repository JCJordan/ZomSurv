...
public class GraphicsServer extends Thread {
	
	private DatagramSocket socket;
	private GraphicsManager gm;
	private List<PlayerMP> connectedPlayers = new ArrayList<PlayerMP>();
	
	public GraphicsServer(GraphicsManager gm) {
		this.gm = gm;
		try {
...
//			}
	...
	private void parsePacket(byte[] data, InetAddress address, int port) {
		...
		case LOGIN:
			...
			PlayerMP player = new PlayerMP(gm.level, 100, 100, ((Packet00Login)packet).getUsername(), address, port);			
		...
		case MOVE:
			...
			//System.out.println(((Packet02Move)packet).getUsername() + " has moved to " + ((Packet02Move)packet).getX() + "," + ((Packet02Move)packet).getX());
